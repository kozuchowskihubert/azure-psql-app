# User Guides

This directory contains user-facing guides for features and functionality.

## 🎨 UI/UX Guides

- **[MODE_SWITCHING_GUIDE.md](./MODE_SWITCHING_GUIDE.md)** - Advanced/Basic mode toggle documentation
- **[UI_SIMPLIFICATION_GUIDE.md](./UI_SIMPLIFICATION_GUIDE.md)** - UI simplification features
- **[UI_UX_CHANGELOG.md](./UI_UX_CHANGELOG.md)** - Visual changelog of UI/UX improvements
- **[UI_UX_ENHANCEMENT_SUMMARY.md](./UI_UX_ENHANCEMENT_SUMMARY.md)** - Summary of UI enhancements
- **[UI_UX_QUICK_REFERENCE.md](./UI_UX_QUICK_REFERENCE.md)** - Quick reference for UI features
- **[NAVIGATION_FIX_SUMMARY.md](./NAVIGATION_FIX_SUMMARY.md)** - Navigation link fixes documentation

## 🎵 Music Production Guides

- **[DRUM_PATTERNS_GUIDE.md](./DRUM_PATTERNS_GUIDE.md)** - Drum pattern creation and sequencing
- **[ARRANGEMENT_BEAT_GENERATOR_GUIDE.md](./ARRANGEMENT_BEAT_GENERATOR_GUIDE.md)** - Intelligent beat generation
- **[COLLABORATIVE_RECORDING_GUIDE.md](./COLLABORATIVE_RECORDING_GUIDE.md)** - Collaborative features

## ⚡ Techno Creator Documentation

- **[TECHNO_CREATOR_QUICK_REFERENCE.md](./TECHNO_CREATOR_QUICK_REFERENCE.md)** - 🆕 Quick reference guide (lean & clean)
- **[TECHNO_CREATOR_PRESET_BUTTON_MAP.md](./TECHNO_CREATOR_PRESET_BUTTON_MAP.md)** - Complete preset and button mapping (detailed)
- **[TECHNO_BUTTON_TEST_CHECKLIST.md](./TECHNO_BUTTON_TEST_CHECKLIST.md)** - Manual testing checklist
- **[TECHNO_VERIFICATION_SUMMARY.md](./TECHNO_VERIFICATION_SUMMARY.md)** - Verification results and production readiness

## 🔥 Trap Studio Documentation

- **[TRAP_STUDIO_QUICK_REFERENCE.md](./TRAP_STUDIO_QUICK_REFERENCE.md)** - 🆕 Quick reference guide (lean & clean)
- **[TRAP_STUDIO_GUIDE.md](../TRAP_STUDIO_GUIDE.md)** - Comprehensive feature guide (detailed)
- **[TRAP_STUDIO_SYNTHESIS.md](../TRAP_STUDIO_SYNTHESIS.md)** - Deep dive into 808 synthesis and sound design
- **[TRAP_STUDIO_PREVIEW.md](../TRAP_STUDIO_PREVIEW.md)** - Feature preview and overview

## 🎼 Music Theory

- **[MUSIC_THEORY_AUDIT_REPORT.md](./MUSIC_THEORY_AUDIT_REPORT.md)** - Comprehensive music theory validation (Trap + Techno)

## 🔗 Related Documentation

- [Technical Documentation](../) - Architecture, API, and development docs
- [Deployment Guides](../deployment/) - Deployment and hosting
- [Main README](../../README.md) - Project overview

